...
class Iptables:
            ...
            if rc == 0:
                if self._ipversion == '4':
                    ...
                    try:
                        ...
                        if rc == 0:
                            if jsonout and 'nftables' in jsonout and len(jsonout['nftables']) > 0:
                                table_names = []
                                for table in jsonout['nftables']:
                                    if 'table' in table and 'name' in table['table']:
                                        table_names.append(table['table']['name'])
                                return table_names
                            else:
                                return self.TABLES
                    except ValueError:  # Catch json.JSONDecodeError for older Python versions
                        # If JSON parsing fails, log the error and return default tables
                        Iptables.module.debug("Failed to parse nft output: %s" % stdout)
                        return self.TABLES
                else:
    ...
    def _check_compatibility(self):
        try:
            ...
            StrictVersion = lambda v: Version(v)
        except ImportError:
